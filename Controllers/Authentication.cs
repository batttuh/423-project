...
namespace YourNamespace.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login(UserLogin request)
        {
            ...
            var posts = await _postRepository.GetPostsByUserId(user.UserID);

            // Generate JWT token
            var token = GenerateJwtToken(user);

            // Return the user and posts along with the token
            return Ok(new { Token = token, User = user, Posts = posts });
        }
